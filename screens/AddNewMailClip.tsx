...
import { SafeAreaView, StyleSheet, Text, View, StatusBar, ScrollView, Pressable } from "react-native";
...
const Component = () => {
   ...
   const handleEmailInput = (key: "selected_email_addresses" | "inboxes", input: string) => {
               ...
               <View style={styles.mainContainer}>
                  <Card>
                     <View style={styles.cardContainer}>
                        ...
                        <RadioButton
                           ...
                           onPress={() => setData({ ...data, selected_email_addresses: "All" })}
                        />
                        <RadioButton
                           title="Specific Email Addresses Only"
                           checked={data.selected_email_addresses !== "All"}
                           onPress={() => setData({ ...data, selected_email_addresses: [] })}
                        />
                        {data.selected_email_addresses !== "All" && <>
                           <View>
                              <Input
                                 placeholder="example@gmail.com"
                                 onSubmit={(val) => handleEmailInput("selected_email_addresses", val)}
                              />
                              <Text style={styles.inputDescription}>add a comma after email address… and  *@company.com to include all emails from a specific company</Text>
                           </View>
                           {data.selected_email_addresses.length > 0 && <View style={{ gap: 10 }}>
                              <Text style={styles.currentText}>Current Email Addresses Selected</Text>
                              {Array.isArray(data.selected_email_addresses) && data.selected_email_addresses.map((email, index) => (
                                 <Text key={index} style={styles.selectedEmailText}>{email}</Text>
                              ))}
                           </View>}
                        </>}
                        <Input
                           ...
                           onChange={(text) => setData({ ...data, word_in_subject: text })}
                        />
                        <Input
                  ...
                  <Card>
                     <View style={styles.cardContainer}>
                        ...
                        <RadioButton
                           ...
                           title="Show 1 Email"
                           onPress={() => setData({ ...data, email_count_to_display: 1 })}
                        />
                        <RadioButton
                           checked={data.email_count_to_display === 3}
                           title="Show 3 Emails"
                           onPress={() => setData({ ...data, email_count_to_display: 3 })}
                        />
                        <RadioButton
                           checked={data.email_count_to_display === 5}
                           title="Show 5 Emails"
                           onPress={() => setData({ ...data, email_count_to_display: 5 })}
                        />
                     </View>
                  </Card>
                  <Card>
                     <View style={styles.cardContainer}>
                        ...
                        <View style={styles.radioButtonsRowContainer}>
                           <RadioButton
                              ...
                              title="Yes"
                              onPress={() => setData({ ...data, push_notification_enabled: true })}
                           />
                           <RadioButton
                              checked={!data.push_notification_enabled}
                              title="No"
                              onPress={() => setData({ ...data, push_notification_enabled: false })}
                           />
                        </View>
                     </View>
                  </Card>
                  <Card>
                     <View style={styles.cardContainer}>
                        ...
                        <View>
                           <Input placeholder="example@gmail.com" onSubmit={val => handleEmailInput("inboxes", val)} />
                           <Text style={styles.inputDescription}>Enter the email address (can be seperated by comma)</Text>
                        </View>
                        <View style={{ gap: 5 }}>
                           {data.inboxes.map((inbox, index) => (
                              <AddedInbox key={index} title={inbox} />
                           ))}
                        </View>
                     </View>
                  </Card>
